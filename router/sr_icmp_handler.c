...
void sr_send_icmp_echo_reply(struct sr_instance *sr, uint8_t *packet, unsigned int len, struct sr_if *dest_iface, struct sr_if *recv_iface )
{
    ...
    memset(icmp_echo_reply_packet, 0, len * sizeof(uint8_t));
    ...
    memcpy(icmp_echo_reply_packet + total_hdr_len, packet + total_hdr_len, 64);
    total_hdr_len = total_hdr_len + 64;
    reply_icmp_hdr->icmp_sum = 0;
    reply_icmp_hdr->icmp_sum = cksum(reply_icmp_hdr, sizeof(sr_icmp_hdr_t) + 64 );

    sr_send_packet(sr, icmp_echo_reply_packet, total_hdr_len, recv_iface->name);
}